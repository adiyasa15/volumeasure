...
type Camera = { id: number; x: number; y: number; wall: WallSide };

function compute(p: ComputeParams) {
  const { roomL, roomW, roomH, stockL, stockW,
    fovH, fovV, tilt, sensorW, sensorH, focalLen, megapix, panEff, olPhoto, olCam, baseB } = p;
...
// ── SVG Top View ─────────────────────────────────────────────────────────────
function TopView({ r }: { r: ComputeResult }) {
...
// ── SVG Sweep View ───────────────────────────────────────────────────────────
function SweepView({ r }: { r: ComputeResult }) {
  ...
  const IRow = ({ y, label, val, vc }: { y: number; label: string; val: string; vc?: string }) => (
    <>
  ...
  return (
    <svg viewBox="0 0 720 390" style={{ width: "100%" }}>
      ...
      <IRow y={229} label="B/Z ratio"         val={String(r.BZ)}          vc={bc}/>
...
// ── SVG Side View ────────────────────────────────────────────────────────────
function SideView({ r }: { r: ComputeResult }) {
  ...
  return (
    <svg viewBox={`0 0 ${VW} ${CH+PT+50}`} style={{ width: "100%" }}>
      ...
      <IRow y={PT+134} label="B/Z"        val={String(r.BZ)} vc={bc}/>
...
export default function CctvPlanner() {
  ...
  const [activeTab, setActiveTab] = useState<"top" | "sweep" | "side">("top");
  ...
  return (
    <div style={{ background: C.bg, color: C.tx, fontFamily: "'Segoe UI', system-ui, sans-serif", fontSize: 13, margin: "-2rem", minHeight: "calc(100dvh - 56px)" }}>

      {/* ── Subheader ─────────────────────────────────────────────────────── */}
      <div style={{ background: C.bg2, borderBottom: `1px solid ${C.bd}`, padding: "10px 20px", display: "flex", alignItems: "center", gap: 12 }}>
        <div style={{ width: 26, height: 26, background: `linear-gradient(135deg,${C.bl} 0%,${C.tl} 100%)`, borderRadius: 6, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, flexShrink: 0 }}>📐</div>
        <div>
          <div style={{ fontSize: 13, fontWeight: 700, letterSpacing: ".02em" }}>CCTV Stockpile Planner</div>
          <div style={{ fontSize: 10, color: C.tx2, fontFamily: "monospace" }}>photogrammetry placement · 80% overlap · B/Z stereo · 180° pan (wall-mounted)</div>
        </div>
      ...
      {/* ── Body ──────────────────────────────────────────────────────────── */}
      <div style={{ display: "grid", gridTemplateColumns: "290px 1fr", minHeight: "calc(100dvh - 106px)" }}>
        ...
        <div style={{ background: C.bg2, borderRight: `1px solid ${C.bd}`, padding: "14px 12px", overflowY: "auto" }}>
          ...
          <Section title="Spesifikasi CCTV PTZ">
            <div style={{ marginBottom: 8 }}>
              ...
              <select
                value={si}
                onChange={e => { setSi(+e.target.value); setFocalLen(SENSORS[+e.target.value].fl); }}
